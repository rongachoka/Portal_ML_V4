...
SELECT 
    ...
    CURRENT_DATE - COALESCE(lb.last_sold, ld.last_sold) AS days_since_sold,
    CASE
        WHEN CURRENT_DATE - COALESCE(lb.last_sold, ld.last_sold) >= 70 
            THEN 'Dead Stock'
...
WHERE 
    COALESCE(lb.last_sold, ld.last_sold) IS NOT NULL
    AND CURRENT_DATE - COALESCE(lb.last_sold, ld.last_sold) >= 70;