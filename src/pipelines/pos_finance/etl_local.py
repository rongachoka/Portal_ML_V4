...
_POS_SLASH_MDY_FORMATS = [
    # Sales-report "Date Sold" is exported in US-style month/day order even
    # though other columns in the same file (for example "Last Sold") use DMY.
    # Prefer MDY here so values like 2/6/26 parse as 2026-02-06, not 2026-06-02.
    "%m/%d/%Y %H:%M:%S",
    "%m/%d/%Y %H:%M",
    "%m/%d/%Y",
    "%m/%d/%y %H:%M:%S",
...
# New columns not present in old files — filled with None for backward compat
_SALES_NEW_COLS = [
...
def _infer_slash_date_order(series: pd.Series) -> str:
    """
    Infer whether slash-style dates in a single POS file are DMY or MDY.

    We only need to inspect values that force one interpretation:
      - first token  > 12  => must be DMY
      - second token > 12  => must be MDY
    """
    ...
    sample = sample[sample.ne("")].head(5000)

    dmy_forced = 0
    mdy_forced = 0
    for value in sample:
        match = re.match(r"^(\d{1,2})/(\d{1,2})/(\d{2,4})(?:\s|$)", value)
        if not match:
            continue
    ...
    return "mdy" if mdy_forced > dmy_forced else "dmy"


def _get_pos_date_formats(series: pd.Series | None = None) -> list[str]:
    slash_order = _infer_slash_date_order(series) if series is not None else "dmy"
    if slash_order == "mdy":
        return _POS_NON_SLASH_DATE_FORMATS + _POS_SLASH_MDY_FORMATS + _POS_SLASH_DMY_FORMATS
    return _POS_NON_SLASH_DATE_FORMATS + _POS_SLASH_DMY_FORMATS + _POS_SLASH_MDY_FORMATS


def _parse_pos_datetime_value(value, formats=None):
    if pd.isna(value):
        return pd.NaT
...
def parse_date_sold(series: pd.Series) -> pd.Series:
    """Parse POS Date Sold values using the slash-order inferred from that file."""
    formats = _get_pos_date_formats(series)
    return series.apply(lambda value: _parse_pos_datetime_value(value, formats=formats))
...
def _make_missing_receipt_ids(count: int, prefix: str) -> list[str]:
    return [f"{prefix}{i}" for i in range(count)]


def _normalize_date_key(series: pd.Series) -> pd.Series:
    dt = pd.to_datetime(series, errors="coerce")
    return dt.dt.normalize().dt.strftime("%Y-%m-%d").fillna("")


def resolve_cashier_rows_for_merge(df_cashier: pd.DataFrame, df_sales: pd.DataFrame) -> pd.DataFrame:
    """
    Prepare cashier rows for a date-aware merge.

    Receipt numbers are not globally unique and can even repeat within the same
    day, so we key them by (transaction_date, receipt). When the same receipt is
    reused on the same day, we try to align the sales-side transaction total to
    the correct cashier row and push the leftovers into cashier-only fallback rows.
    """
    if df_cashier.empty:
        return df_cashier

    combined = df_cashier.copy()
    combined["Receipt Txn No"] = combined["Receipt Txn No"].fillna("").astype(str).str.strip()

    no_id_mask = combined["Receipt Txn No"] == ""
    if no_id_mask.any():
        combined.loc[no_id_mask, "Receipt Txn No"] = _make_missing_receipt_ids(no_id_mask.sum(), "__NOID_")

    combined["Receipt Txn No"] = clean_id(combined["Receipt Txn No"])
    combined["_transaction_date"] = pd.to_datetime(combined["_transaction_date"], errors="coerce").dt.normalize()
    combined["_merge_date"] = _normalize_date_key(combined["_transaction_date"])
    ...
    sales_keys = df_sales.copy()
    sales_keys["_merge_date"] = _normalize_date_key(sales_keys["Date_Obj"])
    sales_keys["Total (Tax Ex)"] = pd.to_numeric(
        sales_keys["Total (Tax Ex)"], errors="coerce"
    ).fillna(0.0)
    sales_totals = (
        sales_keys.groupby(["Transaction ID", "_merge_date"], as_index=False)["Total (Tax Ex)"]
        .sum()
        .rename(columns={"Transaction ID": "Receipt Txn No", "Total (Tax Ex)": "_sales_txn_total"})
    )

    combined = combined.merge(
        sales_totals,
        on=["Receipt Txn No", "_merge_date"],
        how="left",
    )
    combined["_receipt_date_key"] = combined["_merge_date"] + "||" + combined["Receipt Txn No"]
    combined["_cashier_collision_status"] = "unique"

    resolved_groups = []
    dup_counter = 0

    for _, grp in combined.groupby("_receipt_date_key", sort=False, dropna=False):
        if len(grp) == 1:
            resolved_groups.append(grp)
            continue

        expected = grp["_sales_txn_total"].dropna().unique()
        chosen_idx = None
        if len(expected) == 1:
            matches = grp.index[(grp["Amount"] - expected[0]).abs() < 0.01]
            if len(matches) == 1:
        ...
        chosen = grp.loc[[chosen_idx]].copy()
        chosen["_cashier_collision_status"] = "selected_for_merge"
        resolved_groups.append(chosen)

        leftovers = grp.drop(index=chosen_idx).copy()
        if not leftovers.empty:
            leftovers["_cashier_collision_status"] = "duplicate_cashier_only"
            placeholders = _make_missing_receipt_ids(len(leftovers), f"__DUP_{dup_counter}_")
            dup_counter += len(leftovers)
            leftovers["Receipt Txn No"] = placeholders
            leftovers["_receipt_date_key"] = leftovers["_merge_date"] + "||" + leftovers["Receipt Txn No"]
            resolved_groups.append(leftovers)

    combined = pd.concat(resolved_groups, ignore_index=True)
    ...
    combined = combined.groupby(["Receipt Txn No", "_merge_date"], as_index=False).agg(agg)
...
def _read_one_sales_file(path: Path) -> pd.DataFrame:
    ...
    try:
        ...
        # Fill new columns with None before keep — old files won't have them
        for col in _SALES_NEW_COLS:
            if col not in df.columns:
                df[col] = None

        # Keep only the columns we care about (gracefully skips any still missing)
...
def run_pos_etl_local():
    ...
    for branch, location_label in BRANCHES.items():
        ...
        df_sales["Date_Obj"] = pd.to_datetime(df_sales["Date_Obj"], errors="coerce")
        df_sales["_merge_date"] = _normalize_date_key(df_sales["Date_Obj"])
        ...
        if not df_cashier.empty:
            df_cashier = resolve_cashier_rows_for_merge(df_cashier, df_sales)
            ...
            try:
                df_merged = pd.merge(
                    df_sales,
                    df_cashier,
                    left_on=["Transaction ID", "_merge_date"],
                    right_on=["Receipt Txn No", "_merge_date"],
                    how="outer",
                    copy=False,
                )
    ...
    final_df.drop(columns=["_merge_date"], inplace=True, errors="ignore")
    ...
    if "Total (Tax Ex)" in final_df.columns:
        ...
        txn_id = final_df["Transaction ID"].astype(str)
        mask_no_sales = final_df["Transaction ID"].isna() | txn_id.str.strip().eq("")
