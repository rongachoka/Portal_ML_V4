...
def load_fact_lineitems(cur, df: pd.DataFrame) -> int:
    ...
    INSERT_SQL = """
        ...
        ON CONFLICT (location, transaction_id, description, total_tax_ex) DO UPDATE SET
            ...
            loaded_at         = NOW()
        WHERE
            fact_sales_lineitems.ordered_via   IS DISTINCT FROM EXCLUDED.ordered_via
    ...
    # Replace all remaining NaN/NaT with None so psycopg2 sends NULL
    df = df.where(pd.notnull(df), None)

    records = df.to_dict("records")   # C-level; ~10x faster than iterrows()
    rows = [
        (
            r["location"],
            r["transaction_id"],
            ...
            r.get("qty_sold"),
            r.get("total_tax_ex"),
            r["_date_sold_str"],          # raw date string for audit
            r.get("sale_date"),
            r.get("sale_date_str"),
            r.get("client_name"),
            r.get("phone_number"),
            ...
            r.get("txn_type"),
            r.get("ordered_via"),
            r.get("amount"),              # cashier_amount
            r.get("pos_txn_sum"),         # transaction_total = POS sum
            r.get("audit_status"),
        )
        for r in records
    ]
...
def load_fact_transactions(cur, df: pd.DataFrame) -> int:
    ...
    ON CONFLICT (location, transaction_id) DO UPDATE SET
        ...
        loaded_at              = NOW()
    WHERE
        fact_sales_transactions.ordered_via            IS DISTINCT FROM EXCLUDED.ordered_via
    ...
    rows = [
        (
            ...
            r.get("ordered_via"),
            r.get("pos_txn_sum"),
            r.get("cashier_amount"),
            r.get("real_transaction_value"),
            r.get("products_in_txn"),
            r.get("item_count"),
            r.get("audit_status"),
        )
        for r in records
...
def load_fact_inventory_snapshot(cur, df: pd.DataFrame, branch_label: str) -> int:
    ...
    rows = [
        (
            branch_label,
            r.get("snapshot_date"),
            r.get("snapshot_date_source"),
            r.get("department"),
            r.get("category"),
            r.get("item_lookup_code"),
            r.get("description"),
            r.get("on_hand"),
            r.get("committed"),
            r.get("reorder_pt"),
...
def run_pos_loader():
    ...
    for branch in branches_in_staging:
        ...
        df_cashier = read_cashier_staging(engine, branch, after_date=watermark)
