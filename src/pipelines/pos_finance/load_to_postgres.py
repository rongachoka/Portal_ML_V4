    ...
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                ...
                max_date_loaded = row[0]
                today = datetime.now().date()
                if max_date_loaded and max_date_loaded > (today + timedelta(days=1)):
                    print(
                        f"    Ignoring corrupt watermark for {key}: "
                        f"{max_date_loaded} (today is {today})"
                    )
                    return None
                return max_date_loaded
                ...
                cur.execute("""
                    ...
                    ON CONFLICT (branch) DO UPDATE SET
                        max_date_loaded = CASE
                            WHEN fact_load_watermarks.max_date_loaded > CURRENT_DATE + 1
