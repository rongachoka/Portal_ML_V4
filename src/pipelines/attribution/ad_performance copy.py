...
def run_ad_performance():
    ...
    # ── 7. Save performance table ──────────────────────────────────────
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    df_final.to_csv(OUTPUT_PERFORMANCE, index=False)
    print(f"\n✅ fact_ad_performance.csv → {len(df_final):,} ads saved")

    # ── 8. Products drill-through table ───────────────────────────────
    if not df_sales_raw.empty:
        # Add Ad Name onto each line item so Power BI drill-through has
        # the name available without needing a relationship join
    ...
    # ── 10. Per-ad breakdown (top 10 by revenue) ───────────────────────
