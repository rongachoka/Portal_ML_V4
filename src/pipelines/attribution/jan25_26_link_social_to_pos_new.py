...
POS_DATA_PATH = PROCESSED_DATA_DIR / "pos_data" / "all_locations_sales.csv"
OUTPUT_FILE = PROCESSED_DATA_DIR / "sales_attribution" / "attributed_sales_waterfall_v7.csv"

# Attribution only looks at sessions from Jan 2025 onwards.
# The ETL now loads full history — we filter here at read time.
ATTRIBUTION_START_DATE = pd.Timestamp("2025-01-01")
...
def run_attribution_v7():
    ...
    # 1. LOAD DATA
    print("   📥 Loading Data...")
    df_chat = pd.read_csv(CHAT_DATA_PATH, low_memory=False, 
                        dtype={
                            'Ad Name': str,
                            'phone_number': str,
                        })
    # Read POS in chunks with selective columns — full history file can be large
    POS_COLS_NEEDED = [
        'Transaction ID', 'Date Sold', 'Description', 'On Hand',
        'Phone Number', 'Client Name', 'Total (Tax Ex)', 'Amount',
        'Ordered Via', 'Qty Sold', 'Location', 'Sale_Date', 'Transaction_Total',
    ]
    POS_DTYPES = {
        'Transaction ID': str,
        'Phone Number':   str,
    ...
    _header = _pd.read_csv(POS_DATA_PATH, nrows=0).columns.tolist()
    _usecols = [c for c in POS_COLS_NEEDED if c in _header]

    chunks = []
    for chunk in pd.read_csv(
        POS_DATA_PATH,
        usecols=_usecols,
        dtype={k: v for k, v in POS_DTYPES.items() if k in _usecols},
        chunksize=100_000,
        low_memory=False,
    ):
        chunks.append(chunk)
    df_pos_raw = pd.concat(chunks, ignore_index=True)
    del chunks
    gc.collect()

    # Filter to attribution window — ETL holds full history, we only attribute Jan 2025+
    df_pos_raw['POS_DateTime'] = df_pos_raw['Date Sold'].apply(parse_hash_date)
    df_pos_raw = df_pos_raw[
        df_pos_raw['POS_DateTime'] >= ATTRIBUTION_START_DATE
    ].copy()
    print(f"   📅 POS filtered to {ATTRIBUTION_START_DATE.date()} onwards: {len(df_pos_raw):,} rows")
    ...
    # 3. PREP POS
    # POS_DateTime already parsed and filtered above at load time
    ...
    df_pos_headers = df_pos_raw.groupby('Transaction ID', as_index=False).agg({
        ...
        'Total (Tax Ex)': 'sum',
        'Amount': 'max',
        'Ordered Via': 'first',    # Staff-tagged source — key for Layer 0
    })
    ...
    # =========================================================
    # 💎 LAYER 0: PLATINUM (Ordered Via = Respond.io / WhatsApp)
    # Staff explicitly tagged this sale as coming from social media
    # in the cashier report. Strongest possible signal — no inference
    # needed. Still require phone match to link to the right session.
    # =========================================================
    print("   💎 Layer 0: Platinum (Ordered Via = Respond.io/WhatsApp + Phone)...")
    SOCIAL_TAGS = ['respond.io', 'respond', 'whatsapp', 'whats app']

    pos_social = unmatched_pos[
        unmatched_pos['Ordered Via'].fillna('').str.lower().apply(
            lambda v: any(tag in v for tag in SOCIAL_TAGS)
        )
    ].copy()

    if not pos_social.empty and unmatched_chat['norm_phone'].notna().any():
        m0 = pd.merge(
            unmatched_chat.dropna(subset=['norm_phone']),
            pos_social.dropna(subset=['norm_phone']),
            on='norm_phone',
            how='inner'
        )
        if not m0.empty:
            m0['diff_hours'] = (
                m0['POS_DateTime'] - m0['session_start']
            ).dt.total_seconds() / 3600
            m0 = m0[(m0['diff_hours'] > MIN_HOURS) & (m0['diff_hours'] <= MAX_HOURS)]
            m0 = m0.sort_values('diff_hours')
            m0 = m0.drop_duplicates('Transaction ID', keep='first')
            m0 = m0.drop_duplicates('session_id', keep='first')
            if not m0.empty:
                m0['match_type'] = 'Platinum (Ordered Via)'
                m0['confidence'] = 99
                log_match("Platinum", m0)
    ...
    if results:
        ...
        # ── REVENUE BREAKDOWN BY MATCH LAYER ──
        if 'Final_Amount_Inc' in final_df.columns and 'match_type' in final_df.columns:
            from datetime import datetime as _dt

            # Current month from system clock — updates automatically each month
            now         = _dt.now()
            month_label = now.strftime("%B %Y")   # e.g. "March 2026"
            month_start = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)

            # One row per transaction (avoid line-item duplication)
            txn_all = (
                final_df.drop_duplicates(subset=['Transaction ID'])
                [['match_type', 'POS_DateTime', 'Final_Amount_Inc']].copy()
            )
            txn_all['Final_Amount_Inc'] = pd.to_numeric(
                txn_all['Final_Amount_Inc'], errors='coerce'
            ).fillna(0)
            txn_all['POS_DateTime'] = pd.to_datetime(txn_all['POS_DateTime'], errors='coerce')

            txn_month = txn_all[txn_all['POS_DateTime'] >= month_start].copy()

            def _print_table(df, label):
                total_rev   = df['Final_Amount_Inc'].sum()
                total_txns  = len(df)
                print(f"\n   {'─'*72}")
                print(f"   💰  {label}")
                print(f"   {'─'*72}")
                print(f"   {'Layer':<32} {'Txns':>6}  {'Revenue (KES)':>14}  {'Share':>6}")
                print(f"   {'─'*72}")
                for layer in ['Platinum (Ordered Via)', 'Gold (Phone)',
                              'Silver (Name+Price)', 'Bronze (Price+Content)']:
                    grp   = df[df['match_type'] == layer]
                    if grp.empty:
                        continue
                    rev   = grp['Final_Amount_Inc'].sum()
                    count = len(grp)
                    share = (rev / total_rev * 100) if total_rev > 0 else 0
                    print(f"   {layer:<32} {count:>6,}  KES {rev:>11,.0f}  {share:>5.1f}%")
                print(f"   {'─'*72}")
                print(f"   {'TOTAL':<32} {total_txns:>6,}  KES {total_rev:>11,.0f}  100.0%")

            _print_table(txn_all,   "CUMULATIVE (All Time)")
            _print_table(txn_month, f"THIS MONTH ({month_label})")
        else:
            print(df_links['match_type'].value_counts())

        # ── WRITEBACK: stamp ordered_via onto fact_sessions_enriched ──
        # This is what makes classify_source in analytics.py actually work.
        # Without this, the ordered_via column in fact_sessions_enriched is
        # always empty and the waterfall attribution logic never fires.
        print("\n   📝 Writing ordered_via back to fact_sessions_enriched...")
        try:
            df_sessions_full = pd.read_csv(CHAT_DATA_PATH)

            # Build a session_id → ordered_via map from matched results
            ordered_via_map = (
                final_df[['session_id', 'Ordered Via']]
                .dropna(subset=['Ordered Via'])
                .drop_duplicates('session_id')
                .set_index('session_id')['Ordered Via']
                .to_dict()
            )

            # Only update rows that were matched — don't overwrite existing values
            if 'ordered_via' not in df_sessions_full.columns:
                df_sessions_full['ordered_via'] = None

            mask = df_sessions_full['session_id'].isin(ordered_via_map)
            df_sessions_full.loc[mask, 'ordered_via'] = (
                df_sessions_full.loc[mask, 'session_id'].map(ordered_via_map)
            )

            df_sessions_full.to_csv(CHAT_DATA_PATH, index=False)
            stamped = mask.sum()
            print(f"      ✅ {stamped:,} sessions updated with ordered_via")
        except Exception as e:
            print(f"      ⚠️  Writeback failed (non-fatal): {e}")
