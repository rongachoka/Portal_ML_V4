...
def run_attribution_v7():
    ...
    # 🟢 Only attempt Silver on chat sessions with meaningful amounts
    subset_chat = unmatched_chat[unmatched_chat['mpesa_amount'] > 500].copy()

    # 🟢 Slim POS to only columns needed for Silver — reduces merge footprint
    pos_silver = unmatched_pos[['Transaction ID', 'POS_DateTime', 'norm_phone', 
                                'Client Name', 'Final_Amount_Inc', 'amt_key',
                                'Full_Basket_Desc']].copy()

    m2 = pd.merge(
        subset_chat, pos_silver, on='amt_key',
    ...
    subset_chat = unmatched_chat[unmatched_chat['amt_key'] > 500].copy()

    # 🟢 Same slim approach
    pos_bronze = unmatched_pos[['Transaction ID', 'POS_DateTime', 'norm_phone',
                                'Client Name', 'Final_Amount_Inc', 'amt_key',
                                'Full_Basket_Desc']].copy()

    m3 = pd.merge(
        subset_chat, pos_bronze, on='amt_key',
        how='inner', suffixes=('_chat', '_pos')
    )
