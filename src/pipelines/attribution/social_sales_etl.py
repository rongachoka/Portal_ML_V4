...
Pipeline dependency:
    etl_local.py → all_locations_sales_NEW.csv → this script → social_sales_etl.csv
...
OUTPUT_FILE          = OUTPUT_DIR / "social_sales_etl.csv"
CONCERN_OUTPUT_FILE  = OUTPUT_DIR / "social_sales_etl_by_concern.csv"
CATEGORY_OUTPUT_FILE = OUTPUT_DIR / "social_sales_etl_by_category.csv"
...
KEEP_COLS = [
    ...
    "Phone Number", "Sales Rep",          # from cashier — used for social attribution
    "Sales Rep ID", "Sales Rep Name",     # from sales file — used for staff performance
    "acquisition_source",
...
def load_ads_for_pos() -> pd.DataFrame:
    ...
    # 4. Map Ad Name — only if Ad ID column exists in the lookup
    if "Ad ID" in df_lookup.columns:
...
def run_social_sales_etl():
    """
    Entry point — function name kept identical to social_sales_etl.py so
    any pipeline runner calling run_social_sales_etl() needs no changes.
    """
    print("=" * 65)
    ...
    # ── 2. Filter to respond.io cashier rows only ──────────────────────────
    mask = (
        df["Ordered Via"]
    ...
    if df_social.empty:
        print("\n❌  No social-tagged rows found. Check 'Ordered Via' column values:")
    ...
    print(f"\n📊  REVENUE GAP REPORT")
    print(f"    social-tagged txns in cashier : {total_txns_cashier:>6,}")
    ...
    df_social["norm_phone"] = None   # always exists; populated below if column present
    ...
    # ── 9. Relabel unmatched brands — mirrors original social_sales_direct.py ─
    # Rows where no brand was detected keep Matched_Brand = "Unknown" from
    # _EMPTY_MATCH. Relabel these (and "General"/"") to "Other Social Sale" so
    # the counting and Power BI filtering are consistent with gap rows.
    df_social["Matched_Brand"] = df_social["Matched_Brand"].apply(
        lambda b: b if b not in ("Unknown", "General", "") else "Other Social Sale"
    )
    df_social["Matched_Category"] = df_social.apply(
        lambda r: r["Matched_Category"]
        if r["Matched_Brand"] != "Other Social Sale" else "Unmatched",
        axis=1,
    )

    # ── 10. Match stage breakdown ──────────────────────────────────────────
    n_total = len(df_social)
    print(f"\n   Total rows kept: {n_total:,}  (no rows dropped — all social-tagged sales included)")
    ...
    # Some KB products carry multiple comma-separated categories.
    # Split to one category per row so Power BI category slicers work correctly.
