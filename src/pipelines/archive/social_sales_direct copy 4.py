...
from Portal_ML_V4.src.config.ad_registry import get_ad_by_name
try:
...
def load_ads_for_pos() -> pd.DataFrame:
    ...
    # Map Campaign Name from AD_REGISTRY (Ad Name -> campaign_name)
    # Filling the registry automatically flows through to the output CSV.
    df_lookup['Campaign Name'] = df_lookup['Ad Name'].apply(
        lambda n: (get_ad_by_name(n) or {}).get('campaign_name', None)
        if pd.notna(n) else None
    )

    registered = df_lookup['Campaign Name'].notna().sum()
    print(f"   ✅ Ad lookup built: {df_lookup['norm_phone'].nunique():,} unique phones with ad history · {registered:,} rows matched to registry")
    return df_lookup
...
def enrich_with_acquisition_source(df: pd.DataFrame, df_ads: pd.DataFrame,
    ...
    for col in ['acquisition_source', 'Ad Name', 'Ad campaign ID', 'Ad ID']:
    ...
    matched = pd.merge_asof(
        txn_keys,
        df_ads_sorted[['norm_phone', 'Timestamp', 'Ad ID', 'Ad campaign ID', 'Ad Name', 'Campaign Name']],
        left_on='Sale_Date',
        right_on='Timestamp',
        by='norm_phone',
    ...
    for col in ['Ad campaign ID', 'Ad ID', 'Ad Name', 'Campaign Name']:
...
def run_social_sales_direct():
    ...
    keep_cols = [
        ...
        "Description", "Qty Sold", "Total (Tax Ex)",
        "Ordered Via", "Client Name", "Phone Number", "Sales Rep",
        "acquisition_source", "Ad Name", "Campaign Name", "Ad campaign ID", "Ad ID",
        "Matched_Brand", "Matched_Product", "Matched_Category",
        "Matched_Sub_Category", "Matched_Concern", "cost_status", "match_stage",
    ]
