...
# 🚨 1. CONTEXT VALIDATION RULES (Kept this, it's safe)
BRAND_CONTEXT_RULES = {
...
# ✅ FOR CONTACT ID & SESSION ID (No Apostrophe - Clean String)
def clean_id(val):
    if pd.isna(val): return None
    s = str(val).strip().replace('.0', '')
    s = ''.join(filter(str.isdigit, s)) # Keep digits only
    if len(s) == 0: return None
    # Standardize 254 -> +254 (Optional, but good for ID matching)
    if s.startswith('254'): return f"+{s}"
    return s

# ✅ FOR EXCEL EXPORT ONLY (Has Apostrophe ' to force text format)
...
# 🚨 REVENUE RECALCULATION REMOVED
# We are skipping the deduplication logic to restore original revenue levels.
    ...
    def get_best_match(self, context, mpesa_amount=0, brand_hint=None):
        ...
        try:
            ...
            if best_match is not None and highest_score > threshold:
                # ✅ CONTEXT VALIDATION (Aptamil Fix)
...
def run_analytics_pipeline():
    print("📊 V46.0 MASTER ANALYTICS: Restored Revenue + Phone Export")
    ...
    print("💰 Deduplicating Revenue (Using M-Pesa Engine)...")

    # Apply the engine to the full context
    df_sess['mpesa_results'] = df_sess['full_context'].apply(lambda x: detect_payment_converted_v2(str(x)))

    # Extract the amount
    df_sess['mpesa_amount'] = df_sess['mpesa_results'].apply(lambda x: x['amount'])

    # Clean up temp column
    df_sess.drop(columns=['mpesa_results'], inplace=True)
    ...
    if os.path.exists(cleaned_cont_path):
        ...
        if id_n:
            df_n[id_n] = df_n[id_n].apply(clean_id) # USE CLEAN ID FOR JOIN KEY
            
            if name_c:
                name_map = df_n.set_index(id_n)[name_c].to_dict()
            
            if phone_c:
                # 🚨 APPLY EXCEL FORMATTING ONLY TO THE VALUE (PHONE), NOT THE KEY
    ...
    print("👥 Mapping Staff...")
    TEAM_MAP = { "847526": "Ishmael", "860475": "Faith", "879396": "Nimmoh", "879430": "Rahab", "879438": "Brenda", "962460": "Katie", "1000558": "Sharon", "845968": "Joy" }
    def map_staff(val):
    ...
    # df_sess['bracket_10_20k_sort'] = bracket_10_20_res.apply(lambda x: x[1]) # Dropped
    ...
    # 1. Add phone_number column using priority map
    df_sess['phone_number'] = df_sess['Contact ID'].map(phone_map)
    # 2. NO FALLBACK (Leave as blank if missing)
