...
PSEUDO_BRANDS = [
    "magnesium", "magnisium", "zinc", "calcium", "biotin", "collagen", "collegen",
    "vitamin", "omega", "cod liver", "fish oil", "multivitamin",
    "panadol", "augmentin", "antibiotic", "painkiller", "maxitrol", "tobrasone", "eye drops",
    "sunscreen", "retinol", "salicylic", "glycolic", "niacinamide", "benzoyl", "peroxide",
    "shampoo", "conditioner", "gel", "cleanser", "moisturizer",
    "effaclar", "zelaton", "acnes"
]
...
def run_analytics_pipeline():
    print("📊 V25.0 ANALYTICS: DUAL GUARDRAILS & GENERIC TRAP")
    ...
    def process_ai(row):
        ...
        if match_row is not None:
            ...
            for sub, keywords in SUB_CAT_RULES_ORDERED.items():
                if any(k in text_lower for k in keywords):
                    p_sub = sub
                    # Only fallback if it's safe (Based on Brand Lists)
        ...
        # ✅ DUAL SAFETY CHECK (Guardrails)
        if p_brand in SKINCARE_ONLY_BRANDS and p_cat not in ["Skincare"]:
            p_cat = "Skincare"
        if p_brand in HAIRCARE_ONLY_BRANDS and p_cat not in ["Hair Care"]:
            p_cat = "Hair Care"
    ...
    # Lifetime Tier (for dim_customers)
    ...
    # ✅ SESSION TIER (Fix for Chart Mix-up)
    ...
    # ✅ NEW: MULTI-PASS PRODUCT EXTRACTION (Catches Multiple Products)
    print("🔍 Running Multi-Pass Product Extraction...")
    p_rows = []
    for _, r in df_sess.iterrows():
    ...
    if p_rows:
        ...
        print(f"   ✅ Exported {len(p_rows)} granular product matches (Multi-Pass) to {output_path}")
