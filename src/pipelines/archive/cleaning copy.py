...
def load_and_prep_ads(ads_folder_path):
    """
    Ingests 60+ Ad files (contacts-added/connected), dedups, and preps for merge.
    """
    all_files = []
    ...
    ad_cols_to_keep = ['Timestamp', 'Contact ID', 'Ad campaign ID', 'Ad group ID', 'Ad ID', 'Source', 'Sub Source']
    dfs = []
    ...
    for f in all_files:
        try:
            ...
            # 🧹 CLEANING: Convert "-" to NaN immediately
            # We treat '-' as a true blank so it doesn't mix with real IDs
            cols_to_clean = ['Ad ID', 'Ad campaign ID', 'Ad group ID']
            for col in cols_to_clean:
                if col in temp.columns:
                    # Replace exactly "-" or " - " with pandas NA
                    temp[col] = temp[col].replace(['-', ' -'], pd.NA)

            # Standardize Timestamp
            temp['Timestamp'] = pd.to_datetime(
                temp['Timestamp'].astype(str).str.strip(),
                format="%Y-%m-%d %H:%M:%S",
                errors="coerce",
                cache=True
            )
            ...
            temp['Contact ID'] = pd.to_numeric(temp['Contact ID'], errors='coerce').astype('Int64')
            
            # Keep only the intersection of columns that exist in this specific file
            available_cols = [c for c in ad_cols_to_keep if c in temp.columns]
            
            # Drop rows with no ID or Time, then select columns
    ...
    # DEDUPLICATION: Strict check on ID + Time + Campaign
    before_dedup = len(df_ads)
    df_ads = df_ads.drop_duplicates(subset=['Contact ID', 'Timestamp', 'Ad campaign ID'])
...
def run_production_cleaning():
    ...
    if not df_ads.empty:
        print("\n🔗 Merging Ad Campaign Data (1-Hour Tolerance)...")
