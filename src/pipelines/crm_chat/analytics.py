...
def load_ads_for_analytics():
    ...
    for f in all_files:
        try:
            ...
            t['Timestamp'] = t['Timestamp'].dt.tz_localize(None)
...
# Lightweight state file — survives fact_sessions_enriched being cleared/renamed
_ANALYTICS_STATE_FILE = PROCESSED_DATA_DIR / "analytics_state.json"


def _get_analytics_watermark(enriched_path) -> pd.Timestamp | None:
    """
    Watermark priority:
      1. analytics_state.json  — fast, survives CSV being cleared
      2. fact_sessions_enriched.csv max session_start  — fallback on first run
         after the state file is introduced, or if state file is missing
    Returns None if neither source exists yet (true first run).
    """
    # 1. State file (preferred)
    if os.path.exists(_ANALYTICS_STATE_FILE):
        try:
            with open(_ANALYTICS_STATE_FILE, "r") as f:
                state = json.load(f)
            ts = state.get("last_processed")
            if ts:
                return pd.Timestamp(ts)
        except Exception as e:
            print(f"⚠️ Could not read analytics_state.json: {e}")

    # 2. Fallback: scan the CSV (covers migration from old runs with no state file)
    if not os.path.exists(enriched_path):
        return None
    try:
        df = pd.read_csv(enriched_path, usecols=['session_start'])
        dates = pd.to_datetime(df['session_start'], errors='coerce').dropna()
        return dates.max() if not dates.empty else None
    except Exception as e:
        print(f"⚠️ Could not read analytics watermark from CSV: {e}")
        return None


def _save_analytics_watermark(max_session_start: pd.Timestamp) -> None:
    """
    Persist the watermark to analytics_state.json so the next run
    knows where to start even if fact_sessions_enriched.csv is cleared.
    """
    try:
        os.makedirs(_ANALYTICS_STATE_FILE.parent, exist_ok=True)
        state = {}
        if os.path.exists(_ANALYTICS_STATE_FILE):
            with open(_ANALYTICS_STATE_FILE, "r") as f:
                state = json.load(f)
        state["last_processed"] = max_session_start.isoformat()
        state["last_run_at"]    = pd.Timestamp.now().isoformat()
        with open(_ANALYTICS_STATE_FILE, "w") as f:
            json.dump(state, f, indent=2)
        print(f"💾 Watermark saved: {max_session_start}")
    except Exception as e:
        print(f"⚠️ Could not save analytics watermark: {e}")


# ==========================================
# 5. MAIN ANALYTICS PIPELINE
# ==========================================
def run_analytics_pipeline(force: bool = False):
    ...
    # ── WATERMARK: skip sessions already enriched ─────────────────────────
    ENRICHED_PATH = PROCESSED_DATA_DIR / "fact_sessions_enriched.csv"
    watermark = _get_analytics_watermark(ENRICHED_PATH)

    if watermark is not None:
        mask_new = df_sess['session_start'] > watermark
        n_new    = mask_new.sum()
        print(
            f"📅 Watermark: {watermark} | "
            f"{n_new:,} new sessions to enrich | "
            f"{(~mask_new).sum():,} already processed — skipping AI"
        )
        if n_new == 0 and not force:
            print("✅ Nothing new since last run.")
            return
        if force:
            print("⚡ Force mode — reprocessing all sessions")
            # Don't filter — process everything
        else:
            df_sess = df_sess[mask_new].copy()
    else:
        print(f"📅 No watermark (first run) — processing all {len(df_sess):,} sessions")
    ...
    if not df_ads.empty:
        ...
        df_sess = pd.merge_asof(
            ...
            by='Contact ID', tolerance=pd.Timedelta('96h'),
    ...
    # Track new session IDs before merging with existing — used for append-only exports
    _new_session_ids = set(df_sess['session_id'].tolist())
    ...
    def _safe_ad_id(val):
        """Converts '6.95968E+12' → '6959681615440' before map lookup."""
    ...
    df_sess['clean_ad_id'] = df_sess.get('Ad ID', pd.NA).apply(_safe_ad_id)
    ...
    # ── PRE-F: reload existing enriched data so cumulative stats are correct ──
    # visit_rank, prior_orders, lifetime tiers all need full history.
    # We only ran AI on new sessions; here we merge them with the already-enriched
    # rows so section F sees the complete picture.
    # ── PRE-F: reload existing enriched data so cumulative stats are correct ──
    if watermark is not None and os.path.exists(ENRICHED_PATH) and not force:
        print("🔗 Merging new sessions with existing enriched data...")
        df_existing = pd.read_csv(ENRICHED_PATH, low_memory=False)
        df_existing['session_start'] = pd.to_datetime(
            df_existing['session_start'], errors='coerce'
        )
        df_existing = df_existing.loc[:, ~df_existing.columns.duplicated()]
        df_sess     = df_sess.loc[:, ~df_sess.columns.duplicated()]
        common_cols = [c for c in df_existing.columns if c in df_sess.columns]
        df_sess = pd.concat(
            [df_existing[common_cols], df_sess[common_cols]],
            ignore_index=True
        )
        df_sess = df_sess.drop_duplicates(subset=['session_id'], keep='last')
        df_sess = df_sess.sort_values(
            ['Contact ID', 'session_start']
        ).reset_index(drop=True)
        print(f"   ✅ Combined: {len(df_sess):,} total sessions for cumulative calc")
    elif force:
        print("⚡ Force mode — skipping merge, rewriting all output files from scratch")

    # Recompute clean_ad_id and Ad Name on the full df_sess (new + existing).
    # Must happen after the merge — existing rows loaded from CSV still carry
    # the raw scientific notation string (e.g. '6.95968E+12') and a blank Ad Name.
    df_sess['clean_ad_id'] = df_sess.get('Ad ID', pd.NA).apply(_safe_ad_id)
    df_sess['Ad Name'] = df_sess['clean_ad_id'].map(AD_NAME_MAP)
    ...
    df_sess.to_csv(ENRICHED_PATH, index=False)
    stats = stats.reset_index()

    # Deduplicate — keep the row with highest monetary value per Contact ID
    stats = stats.sort_values('monetary_value', ascending=False)
    stats = stats.drop_duplicates(subset=['Contact ID'], keep='first')

    stats.to_csv(PROCESSED_DATA_DIR / "dim_customers_rfv.csv", index=False)

    # Persist watermark so next run knows where to start
    _save_analytics_watermark(df_sess['session_start'].max())

    # For all remaining exports: only iterate NEW sessions, then append to existing CSVs
    df_new_only = df_sess[df_sess['session_id'].isin(_new_session_ids)]

    # PRODUCT EXPORT
    PRODUCT_PATH = PROCESSED_DATA_DIR / "fact_product_mentions.csv"
    p_rows = []
    ...
    if p_rows:
        df_p_new = pd.DataFrame(p_rows)
        if watermark is not None and os.path.exists(PRODUCT_PATH):
            df_p_new = pd.concat(
                [pd.read_csv(PRODUCT_PATH), df_p_new], ignore_index=True
            )
        df_p_new.to_csv(PRODUCT_PATH, index=False)

    # BRAND MENTIONS EXPORT
    BRAND_PATH = PROCESSED_DATA_DIR / "fact_brand_mentions.csv"
    b_rows = []
    for _, r in df_new_only.iterrows():
        ...
        if not brands_to_use:
            ...
            for brand in brands_to_use:
                b_rows.append({"session_id": r['session_id'], "Contact ID": r['Contact ID'],
                               ...
                               "is_converted": r['is_converted'], "date": r['session_start']})
    if b_rows:
        df_b_new = pd.DataFrame(b_rows)
        if watermark is not None and os.path.exists(BRAND_PATH):
            df_b_new = pd.concat(
                [pd.read_csv(BRAND_PATH), df_b_new], ignore_index=True
            )
        df_b_new.to_csv(BRAND_PATH, index=False)

    # FUNNEL EXPORT
    FUNNEL_PATH = PROCESSED_DATA_DIR / "fact_funnel_analytics.csv"
    f_rows = []
    for _, r in df_new_only.iterrows():
    ...
    df_f_new = pd.DataFrame(f_rows)
    if watermark is not None and os.path.exists(FUNNEL_PATH):
        df_f_new = pd.concat(
            [pd.read_csv(FUNNEL_PATH), df_f_new], ignore_index=True
        )
    df_f_new.to_csv(FUNNEL_PATH, index=False)

    # ZONE EXPORT
    ZONE_PATH = PROCESSED_DATA_DIR / "fact_session_zones.csv"
    zone_rows = []
    for _, r in df_new_only.iterrows():
        if r.get('zone_name'):
    ...
    if zone_rows:
        df_z_new = pd.DataFrame(zone_rows)
        if watermark is not None and os.path.exists(ZONE_PATH):
            df_z_new = pd.concat([pd.read_csv(ZONE_PATH), df_z_new], ignore_index=True)
        df_z_new.to_csv(ZONE_PATH, index=False)

    # CONCERN EXPORT
    CONCERN_PATH = PROCESSED_DATA_DIR / "fact_session_concerns.csv"
    concern_rows = []
    for _, r in df_new_only.iterrows():
        ...
        for concern, patterns in CONCERN_RULES.items():
            if any(re.search(pat, text_context) for pat in patterns.get('all', []) + patterns.get('chat', [])):
                concern_rows.append({
                    ...
                    "primary_category":   r.get('primary_category', 'General Inquiry'),
                    "sub_category":       r.get('sub_category', 'General'),
                    "revenue":            r.get('mpesa_amount', 0),
                    "is_converted":       r.get('is_converted', 0),
                    "acquisition_source": r.get('acquisition_source', 'Unknown'),
                    "channel_name":       r.get('channel_name', 'Unknown'),
                    "active_staff":       r.get('active_staff', 'Unmapped'),
                })
    if concern_rows:
        df_c_new = pd.DataFrame(concern_rows)
        if watermark is not None and os.path.exists(CONCERN_PATH):
            df_c_new = pd.concat([pd.read_csv(CONCERN_PATH), df_c_new], ignore_index=True)
        df_c_new.to_csv(CONCERN_PATH, index=False)
...
if __name__ == "__main__":
    ...
    run_analytics_pipeline(force=force)