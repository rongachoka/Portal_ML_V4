...
def run_journey_analysis():
    print("🕵️  STARTING CONCERN JOURNEY ANALYSIS (V22 - Multi-Concern Support)...")
    ...
    df_catalog = df_catalog.dropna(subset=['Name']) # added
    df_catalog['Name'] = df_catalog['Name'].astype(str) # addeed
    ...
    for _, row in df_catalog.iterrows():
        ...
        # 🚨 NEW LOGIC: Parse Multiple Concerns
        # Input: "Acne, Hyperpigmentation, Oily Skin"
        # Output: ['Acne', 'Hyperpigmentation', 'Oily Skin']
        raw_concerns = str(row.get('Concerns', 'General')).split(',')
        clean_concerns = [c.strip() for c in raw_concerns if c.strip()]
        ...
        if anchors:
            product_lookup.append({
                ...
                'concerns': clean_concerns # Store the list here
            })
                ...
                if concern_found:
                    ...
                    if found_recs:
                        ...
                        for prod in unique_recs:
                            # 🚨 NEW LOGIC: Relevance Check
                            # Check if the Detected Concern is inside the Product's Concern List
                            is_relevant = "Check Manual"
                            prod_concerns_str = ", ".join(prod['concerns'])
                            
                            if concern_found:
                                # Simple partial match check
                                if any(concern_found.lower() in c.lower() for c in prod['concerns']):
                                    is_relevant = "High"
                                elif "General" in prod['concerns']:
                                    is_relevant = "General Product"
                                else:
                                    is_relevant = "Potential Mismatch"
                            ...
                            journey_rows.append({
                                ...
                                "Product Targets": prod_concerns_str, # Shows what the product is for
                                "Relevance Score": is_relevant,       # Shows if it matches the complaint
