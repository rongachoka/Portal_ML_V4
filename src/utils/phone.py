...
def _fmt_us(d):
    return f"+1{d[1:4]}{d[4:7]}{d[7:]}"       # +12125550123
def _fmt_uk(d):
    return f"+44{d[2:6]}{d[6:9]}{d[9:]}"       # +447700900982
...
def normalize_phone(val):
    ...
    # ── Strip leading zeros before any prefix check ──────────────
    # Handles 00254XXXXXXXXX (international dialing prefix)
    # e.g. 0025472931756 → 25472931756 → treated as +254 Kenyan
    # Also handles accidental double-zeros on local numbers
    s = s.lstrip('0') or s  # keep at least one char if all zeros
