...
DATE_LOWER = pd.Timestamp("2026-01-01")
...
def load_ads_for_pos() -> pd.DataFrame:
    ...
    if 'Ad ID' in df_ads.columns:
        ...
        df_ads['Ad Name'] = clean_id.map(AD_NAME_MAP)
...
def enrich_with_acquisition_source(
    ...
    if path1_mask.any():
        ...
        matched_p1 = pd.merge_asof(
            txn_p1,
            df_ads.sort_values('Timestamp')[
                ['Contact ID', 'Timestamp', 'Ad ID', 'Ad campaign ID', 'Ad Name']
            ],
            left_on='Sale_Date',
            right_on='Timestamp',
            by='Contact ID',
    ...
    if path2_mask.any() and sessions_path.exists():
        ...
        df_sess['phone_number']  = df_sess['phone_number'].str.strip()
        df_sess = df_sess.dropna(subset=['Contact ID', 'phone_number', 'session_start'])
        ...
        merged_sess = pd.merge_asof(
            txn_p2,
            df_sess[['phone_number', 'session_start', 'Contact ID']],
            left_on='Sale_Date',
        ...
        matched_sess['session_date'] = matched_sess['session_start'].dt.normalize()

        if not matched_sess.empty:
            # Build ads lookup keyed by (Contact ID, calendar_date)
            # Take the last ad event per contact per day (most recent timestamp)
            df_ads_dated = df_ads.copy()
            df_ads_dated['ad_date'] = df_ads_dated['Timestamp'].dt.normalize()
            ads_keyed = (
                df_ads_dated
                .sort_values('Timestamp')
                .drop_duplicates(subset=['Contact ID', 'ad_date'], keep='last')
                .set_index(['Contact ID', 'ad_date'])
                [['Ad ID', 'Ad campaign ID', 'Ad Name']]
            )

            # For each matched session, look up ads on exactly that calendar day
            rows_updated = []
            for _, sess_row in matched_sess.iterrows():
                key = (sess_row['Contact ID'], sess_row['session_date'])
                if key in ads_keyed.index:
                    ad_info = ads_keyed.loc[key]
                    rows_updated.append({
                        'Transaction ID':  sess_row['Transaction ID'],
                        'Ad ID':           ad_info.get('Ad ID'),
                        'Ad campaign ID':  ad_info.get('Ad campaign ID'),
                        'Ad Name':         ad_info.get('Ad Name'),
                    })

            if rows_updated:
                updates = pd.DataFrame(rows_updated).set_index('Transaction ID')
                for col in ['Ad campaign ID', 'Ad ID', 'Ad Name']:
...
def run_social_sales_direct():
    ...
    # 2. Filter to social media sales — exact match on respond.io
    ...
    # 6. Keep ALL respond.io rows — label unmatched as "Other Social Sale"
