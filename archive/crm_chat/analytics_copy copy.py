...
from Portal_ML_V4.src.config.settings import (
    ...
    MSG_HISTORY_RAW, META_ADS_DIR, KB_PATH
)
...
CHANNEL_MAP = {
    ...
    388267: 'TikTok', 389086: 'Web Chat', 492754: 'WhatsApp'
}
...
# With the new meta ads csv export, this will provide more enrichment compared
# to the static AD_NAME_MAP, which is now deprecated but left in place as a fallback.
# but its commented out for now
...
def load_meta_ad_name_map() -> dict:
    """Builds Ad ID → Ad Name map from Meta export. Replaces static AD_NAME_MAP."""
    if not Path(META_ADS_DIR).exists():
        print(f"   ⚠️ Meta ads file not found at {META_ADS_DIR} — Ad Names will be blank")
        return {}
    try:
        df_meta = pd.read_csv(META_ADS_DIR, dtype=str)
        df_meta.columns = df_meta.columns.str.strip()
        df_meta['Ad ID'] = df_meta['Ad ID'].apply(clean_scientific_id)
        ...
        print(f"   ⚠️ Could not build Meta ad name map: {e}")
        return {}
...
def load_ads_for_analytics():
    ...
    df = pd.concat(dfs, ignore_index=True).sort_values('Timestamp', ascending=True)

    # Clean scientific notation from ID columns
    ...
    # ── Map Ad Names from Meta export (replaces static AD_NAME_MAP) ───────────
...
#         )
#         organic_contact_ids = set(
#             df[df['Source'].str.lower().str.strip().isin(['incoming message', 'echo message'])]['Contact ID']
#             .dropna().astype(int).tolist()
#         )
#     else:
#         paid_contact_ids = set(
...
def run_analytics_pipeline():
    ...
    if not df_ads.empty:
        ...
        # FIX: normalize Contact ID to Int64 on both sides before merge_asof.
        # df_sess comes from parquet as float64, df_ads is Int64.
        # Type mismatch in 'by' parameter causes zero matches silently —
        # Ad campaign ID never attaches and Tier 2 never fires for anyone.
    ...
    if not df_c_subset.empty:
        ...
        def resolve_staff_temp(row):
            ...
            for c in candidates:
                ...
                if clean in TEAM_MAP_INTERNAL: 
                    return TEAM_MAP_INTERNAL[clean]
    ...
    def classify_source(row):
        ...
        # TIER 1: Ads folder Source column — strongest signal
        # 'Paid Ads' = clicked an ad. 'Incoming Message' / 'Echo Message' = organic.
        try:
        ...
        # TIER 2: Ad campaign ID or Ad ID present — reliable paid signal
        # Any session where these are populated means the customer clicked an ad
        # to message us. Blank = organic for that session.
        ad_campaign = str(row.get('Ad campaign ID', '')).strip()
        ad_id       = str(row.get('Ad ID', '')).strip()
        if (ad_campaign not in ('', 'nan', 'None', '<NA>') or
                ad_id not in ('', 'nan', 'None', '<NA>')):
            return 'Paid Ads'

        # TIER 3: Conversation history (Opened By Source)
    ...
    df_sess['Ad Name'] = existing_ad_names.fillna(
        df_sess['clean_ad_id'].map(meta_ad_name_map)
    )
    ...
    # ── Stats (lifetime) ──────────────────────────────────────────────────────────
    stats['lifetime_tier_history'] = stats['monetary_value'].apply(get_lifetime_tier)
    stats['lifetime_tier_sort']    = stats['lifetime_tier_history'].map(LIFETIME_TIER_SORT)
    stats['lifetime_bracket']      = stats['monetary_value'].apply(get_broad_bracket)
    stats['lifetime_bracket_sort'] = stats['lifetime_bracket'].map(BROAD_BRACKET_SORT)

    # ── Sessions (per-session spend) ──────────────────────────────────────────────
    df_sess['session_bracket_broad'] = df_sess['mpesa_amount'].apply(get_broad_bracket)
    df_sess['broad_sort']            = df_sess['session_bracket_broad'].map(BROAD_BRACKET_SORT)
