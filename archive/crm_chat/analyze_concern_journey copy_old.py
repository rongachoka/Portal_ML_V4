...
# If Product Name contains KEY, Text MUST contain VALUE.
STRICT_IDENTITY_MAP = {
    # Variants
    'baby': 'baby',
    'kid': 'kid',
    ...
    'snail': 'snail',
    'patches': 'patches',
    'medicated': 'medicated', # Fixes Effaclar Medicated
    
    # Critical Ingredients/Types (Prevents Arbutin vs Matrixyl vs Retinol)
    'arbutin': 'arbutin',
    'niacinamide': 'niacinamide',
    'retinol': 'retinol',
    'retinoid': 'retinoid',
    'matrixyl': 'matrixyl', # Fixes Ordinary Matrixyl
    'peptide': 'peptide',
    'b5': 'b5',
    'salicylic': 'salicylic',
    'glycolic': 'glycolic',
    ...
    # Form Factor Locking (Prevents Serum vs Cleanser Mixup)
    'serum': 'serum',
    'cleanser': 'cleanser',
    'wash': 'wash',
    'toner': 'toner',
...
def check_identity_mismatch(product_name, text):
    """
    STRICT IDENTITY CHECK V21.
    Matches partial words (e.g. 'retinoid' matches 'retinol') via string inclusion.
    """
    p_name_lower = product_name.lower()
    text_lower = text.lower()
    
    for token, required_text in STRICT_IDENTITY_MAP.items():
        # If product has the token (e.g. 'matrixyl')
        if token in p_name_lower:
            # And text DOES NOT have it -> REJECT
...
def find_products_iterative_v21(text, product_lookup, known_brands_set, context_brands, cust_query=""):
    """
    V21: Strict Price + Strict Identity.
    """
    ...
    for _ in range(5):
        ...
        for brand in active_brands:
            ...
            for prod in brand_candidates:
                ...
                # 1. Price Match (Strict 2% for high value, 5% for low)
                price_hit = None
                if mentioned_prices:
...
def run_journey_analysis():
    ...
    df_msg = df_msg[df_msg['Contact ID'].isin(df_sess['Contact ID'].unique())]
    df_msg = df_msg.sort_values(['Contact ID', 'Date & Time'])
    ...
    for contact_id, group in df_msg.groupby('Contact ID'):
