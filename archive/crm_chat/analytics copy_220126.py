...
from Portal_ML_V4.src.pipelines.crm_chat import faq_report
...
MACRO_TO_SUB_CATS = {
    ...
    "Supplements": [
        'vitamin', 'multivitamin', 'iron', 'calcium', 'magnesium', 'biotin',
...
SUB_CAT_RULES_ORDERED = {
    ...
    'Vitamins': ['vitamin', 'multivitamin', 'iron', 'calcium', 'magnesium'],
    'Shampoo': ['shampoo', 'wash'],
}

# 2. PSEUDO-BRANDS
PSEUDO_BRANDS = [
    "panadol", "effaclar", "zelaton"
]
...
# ==========================================
# 1. KNN PRODUCT MATCHER (INTENT-SORTED)
# ==========================================
class ProductMatcher:
    ...
    def _clean_context_smart(self, text):
        ...
        # Re-inject URL slugs (high value)
                ...
                if cat_price > 0:
                    for p in target_prices:
                        ...
                        if abs(cat_price - p) < 50: 
                            ...
                            price_match = True
                            break
                        elif 1.8 <= (p / cat_price) <= 2.2: # Bundle x2
                             similarity += 0.10 
                             price_match = True
                             break
...
# 2. HELPER FUNCTIONS
# ==========================================
# (Standard helpers remain the same: clean_id, find_id_col, split_tags_logic, extract_meta, get_category_from_brand_map)
def clean_id(val):
...
# ✅ NEW: INTENT-BASED SORTING
def normalize_brands_with_intent(brand_str, full_text):
    """
    Returns a SORTED list of brands based on user intent.
    High priority: Brands in URLs, or near 'buy', 'price'.
    Low priority: Brands near 'using', 'used', 'currently'.
    """
    found_brands = set()
    ...
    for brand in found_brands:
        ...
        # Check URLs (Highest Priority)
        if b_clean.replace(" ", "-") in text_lower: # e.g. la-roche
            score += 5
            
        # Scan window around brand
...
def run_analytics_pipeline():
    print("📊 V18.0 ANALYTICS: INTENT SORTING & BRAND PRIORITY")
    ...
    # ✅ CALCULATE LIFETIME BRACKET (For Scatter Charts)
    # This aligns with Customer Tier (Gold = 7-20k)
    stats['lifetime_bracket'] = stats['monetary_value'].apply(get_broad_bracket)

    # Merge Lifetime Stats to Session Data
    df_sess = df_sess.merge(stats[['customer_tier', 'lifetime_bracket']], on='Contact ID', how='left')

    # 3. SESSION BRACKETS (For Histograms/Distribution)
    # This reflects what they bought *today*
    ...
    p_rows = []
    for _, r in df_sess.iterrows():
        prod_name = str(r.get('matched_product', 'Unknown'))
        brand_name = str(r.get('matched_brand', 'Unknown'))
        
        # FILTER LOGIC:
        # 1. Must not be "Unknown"
        # 2. Must not be the generic AI fallback "General [Brand] Inquiry"
        if prod_name != "Unknown" and not prod_name.startswith("General "):
             p_rows.append({
                "session_id": r['session_id'],
                "Contact ID": r['Contact ID'],
                "brand_name": brand_name,
                "product_name": prod_name,       # The specific SKU
    ...
    if p_rows:
        output_path = PROCESSED_DATA_DIR / "fact_product_mentions.csv"
        pd.DataFrame(p_rows).to_csv(output_path, index=False)
        print(f"   ✅ Exported {len(p_rows)} specific product matches to {output_path}")
