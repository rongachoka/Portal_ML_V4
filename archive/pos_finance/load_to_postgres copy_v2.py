...
def run_pos_loader():
    ...
    for folder_name, branch_keywords in LOCATION_MAP.items():
        ...
        if 'Total (Tax Ex)' in df_merged.columns and 'Transaction ID' in df_merged.columns:
            ...
            df_merged['Transaction_Total'] = np.where(
                df_merged['Amount'] != 0, df_merged['Amount'], df_merged['POS_Txn_Sum']
            )
...
# ---------------------------------------------------------
    ...
    print(f"🔌 Connecting to PostgreSQL: {DB_CONFIG['database']} @ {DB_CONFIG['host']}...")
    ...
    with psycopg2.connect(**DB_CONFIG) as conn:
        ...
        with conn.cursor() as cur:
            cur.execute("TRUNCATE TABLE fact_sales_transactions;")
            print("   ✅ fact_sales_transactions truncated")

    print("📤 Loading CSV to database (chunked)...")
    total_loaded = 0
    first_chunk = True

    INSERT_SQL = """
        INSERT INTO fact_sales_transactions (
            location, transaction_id, sale_date, client_name, phone_number,
            sales_rep, txn_type, ordered_via, pos_txn_sum, cashier_amount,
            real_transaction_value, products_in_txn, item_count, audit_status
        ) VALUES %s
    """

    with psycopg2.connect(**DB_CONFIG) as conn:
        with conn.cursor() as cur:
            for chunk in pd.read_csv(OUTPUT_FILE, low_memory=False, chunksize=50000):
                chunk.columns = (
                ...
                if first_chunk:
                    print("CSV columns after normalization:", chunk.columns.tolist())
                    first_chunk = False

                db_chunk = pd.DataFrame({
                    'location':               chunk.get('location'),
                    'transaction_id':         chunk.get('transaction_id'),
                    'sale_date':              pd.to_datetime(chunk.get('sale_date'), errors='coerce'),
                    'client_name':            chunk.get('client_name'),
                    'phone_number':           chunk.get('phone_number'),
                    'sales_rep':              chunk.get('sales_rep'),
                    'txn_type':               chunk.get('txn_type'),
                    'ordered_via':            chunk.get('ordered_via'),
                    'pos_txn_sum':            pd.to_numeric(chunk.get('total__tax_ex_'), errors='coerce').fillna(0),
                    'cashier_amount':         pd.to_numeric(chunk.get('amount'), errors='coerce').fillna(0),
                    'real_transaction_value': pd.to_numeric(chunk.get('transaction_total'), errors='coerce').fillna(0),
                    'products_in_txn':        chunk.get('description'),
                    'item_count':             pd.to_numeric(chunk.get('qty_sold'), errors='coerce').fillna(0),
                    'audit_status':           chunk.get('audit_status'),
                })

                # Convert to list of tuples, replacing NaN/NaT with None
                rows = [
                    tuple(None if pd.isna(v) else v for v in row)
                    for row in db_chunk.itertuples(index=False)
                ]
    ...
    print(f"✅ Database load complete: {total_loaded:,} rows in fact_sales_transactions")
    ...
    print("\n🚀 FULL PIPELINE COMPLETE — DB is up to date.")
