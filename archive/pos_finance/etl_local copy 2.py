...
def _parse_pos_datetime_value(value):
    if pd.isna(value):
        return pd.NaT
...
def parse_date_sold(series: pd.Series) -> pd.Series:
    """Parse POS Date Sold values with explicit formats to avoid DMY/MMY flips."""
    return series.apply(_parse_pos_datetime_value)
