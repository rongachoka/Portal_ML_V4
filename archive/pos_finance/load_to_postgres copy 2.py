...
def load_transactions_to_db(df: pd.DataFrame, engine):
    ...
    Uses ON CONFLICT DO UPDATE (upsert) so re-runs are safe.
    The UNIQUE constraint on (location, transaction_id) handles deduplication.
    """
    if df.empty:
        print("      ⚠️  No transactions to load.")
        return

    # pandas to_sql doesn't support ON CONFLICT natively
    # so we use a temp table + INSERT ... ON CONFLICT approach
    tmp_table = "_tmp_transactions"

    with engine.begin() as conn:
        # Write to temp table
        df.to_sql(tmp_table, conn, if_exists='replace', index=False, method='multi', chunksize=5000)

        # Upsert from temp into real table
        cols = [c for c in df.columns if c not in ('id', 'loaded_at')]
        col_list    = ', '.join(cols)
    ...
    print(f"      ✅ {len(df):,} transaction rows upserted into {TABLE_TRANSACTIONS}")
...
def run_pos_loader(source_file: Path = None, mode: str = 'upsert'):
    ...
    # --- Read ---
    print(f"   📂 Reading: {file_to_load.name}")
    df_raw = pd.read_csv(file_to_load, low_memory=False)
    print(f"      ✅ {len(df_raw):,} rows read")

    # --- Rename columns ---
    df = rename_csv_cols(df_raw)

    # --- Build both grain levels ---
    print("\n   🔨 Building line items...")
    df_lineitems = build_lineitems(df)
    print(f"      ✅ {len(df_lineitems):,} line item rows prepared")

    print("\n   🔨 Building transactions...")
    df_transactions = build_transactions(df_lineitems)
    print(f"      ✅ {len(df_transactions):,} transaction rows prepared")

    # --- Diagnostics before loading ---
    print("\n   📊 Load Preview:")
    print(f"      Locations       : {df_lineitems['location'].nunique()}")
    print(f"      Date range      : {df_lineitems['sale_date'].min()} → {df_lineitems['sale_date'].max()}")
    print(f"      Unique clients  : {df_lineitems['phone_number'].nunique():,}")
    print(f"      Total revenue   : KES {df_transactions['real_transaction_value'].sum():,.0f}")
    ...
    try:
        ...
        print(f"\n   📥 Loading line items ({mode} mode)...")
        load_lineitems_to_db(df_lineitems, engine, mode=mode)

        print(f"\n   📥 Loading transactions (upsert)...")
        load_transactions_to_db(df_transactions, engine)

        refresh_materialized_views(engine)

        print(f"\n✅ POS LOADER COMPLETE.")
        print(f"   fact_sales_lineitems    : {len(df_lineitems):,} rows")
        print(f"   fact_sales_transactions : {len(df_transactions):,} rows")
