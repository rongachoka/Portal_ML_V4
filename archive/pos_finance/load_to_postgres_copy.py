...
def read_sales_staging(engine, branch: str) -> pd.DataFrame:
    """
    Read all rows for a branch from stg_sales_reports.
    Columns already cleaned by sharepoint_parser.py.
    """
    sql = text("""
        SELECT
            branch,
...
def read_cashier_staging(engine, branch: str) -> pd.DataFrame:
    """
    Read all rows for a branch from stg_cashier_reports.
    Time is stored as plain text — no parsing applied here.
    """
    sql = text("""
        SELECT
            branch,
...
def load_fact_transactions(cur, df: pd.DataFrame) -> int:
    ...
    agg = (
        df.groupby(["location", "transaction_id"], as_index=False)
        .agg(
            sale_date              = ("sale_date",              "first"),
            sale_date_str          = ("sale_date_str",          "first"),
            sale_time              = ("sale_time",              "first"),   # 🟢 NEW
            sale_datetime          = ("sale_datetime",          "first"),   # 🟢 NEW
    ...
    rows = [
        tuple(None if (v is None or (isinstance(v, float) and np.isnan(v))) else v
              for v in row)
...
def run_pos_loader():
    ...
    for branch in branches_in_staging:
        ...
        # A. Read from staging
        df_sales   = read_sales_staging(engine, branch)
    ...
    with get_conn() as conn:
        ...
        with conn.cursor() as cur:
            cur.execute("REFRESH MATERIALIZED VIEW CONCURRENTLY mv_transaction_master;")
            print("     mv_transaction_master refreshed")
            cur.execute("REFRESH MATERIALIZED VIEW CONCURRENTLY mv_client_list;")
