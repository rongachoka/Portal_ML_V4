...
def extract_date_from_filename(filename):
    ...
    # Pattern 2: DDMMYY or DMMYY (e.g., 060226, 2701026 -> typo handling?)
    # We look for 6 digits at end or start
    # e.g., s140126 -> 140126
...
def load_sales_files_for_branch(folder_path):
    ...
    # Sort files: Try to parse date from filename to sort chronologically
    def sort_key(fpath):
    ...
    for f in all_files:
        ...
        try:
            ...
            if 'Transaction ID' in df.columns and 'Date Sold' in df.columns:
                ...
                # Filter by Date INSIDE the file (Source of Truth)
...
def optimize_memory(df):
    """
    Downcasts columns to save RAM.
    Run this on your dataframes immediately after loading them.
    """
    for col in df.columns:
        col_type = df[col].dtype
        
        # Convert Objects (Strings) to Categories
        if col_type == 'object':
...
# df_sales = optimize_memory(df_sales)  <-- Add this!

# ==========================================
# 4. MAIN PIPELINE
# ==========================================
def run_pos_etl_v3():
    print("🌍 STARTING POS ETL V4 (Smart Date + Transaction Total)...")
    ...
    for folder_name, branch_keywords in LOCATION_MAP.items():
        ...
        # B. LOAD CASHIER
        # Logic: Load "Current" monthly reports + Historical archives
        current_files = list(loc_path.glob("*Cashier*.xlsm")) + list(loc_path.glob("*Cashier*.xlsx"))
        
        # Load all available cashier files found in the folder
        cashier_dfs = []
    ...
    if master_list:
        ...
        # This is what enables the "Split Payment" matching later
