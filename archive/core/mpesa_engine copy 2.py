...
def detect_payment_converted_v2(text: str) -> dict:
    ...
    noise_patterns = [
        ...
        # Mask Transaction Costs (Handles "Transaction cost, Ksh20.00")
        ...
        # Mask Delivery Estimates mentioned as "It's 450" if confirmed isn't near
        # (This is risky, so we rely on the specific M-Pesa patterns below first)
    ]
    ...
    if not found_payments:
        ...
        if is_confirmed and amounts:
            try:
                ...
                if valid_amts:
                    final_amt = valid_amts[-1] # Assume the last spoken money was the payment
