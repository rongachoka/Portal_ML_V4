...
def detect_brands(text: str) -> Set[str]:
    ...
    t_lower = text.lower()
    found = set()
    # Basic check - optimized for speed
    for brand in KNOWN_BRANDS:
        if brand in t_lower:
            # Verify with word boundary to avoid partial matches (e.g. 'era' in 'camera')
            if re.search(r"\b" + re.escape(brand) + r"\b", t_lower):
                # Return the capitalized version from BRAND_LIST if possible, else title case
...
def infer_concerns_from_text(text: str) -> Set[str]:
    """Uses the V2 Rule Engine to find concerns (Acne, Sleep, etc.)"""
    # This now returns clean names like 'Acne' directly, no 'Concern:' prefix
