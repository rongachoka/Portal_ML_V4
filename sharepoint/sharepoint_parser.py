...
SALES_COLUMNS = [
    ...
    "On Hand", "Unit Cost", "Last Sold", "Qty Sold", "Total (Tax Ex)",
    "Tax Amount", "Total Sales Amount", "Total Cost",
...
# New columns introduced in the updated POS export.
# Filled with None for any file that predates the format change.
SALES_NEW_COLUMNS = [
...
def get_max_date_sold(path: Path) -> date | None:
    ...
    try:
        ...
        formats, _ = _detect_sales_datetime_formats(path, df["Date Sold"])
        dates = df["Date Sold"].apply(lambda value: _clean_date_sold(value, formats)).dropna()
...
def get_branch_watermark(branch: str) -> tuple[date | None, datetime | None]:
    ...
    try:
        ...
        max_date_loaded = row["max_date_loaded"]
        max_datetime_loaded = row["max_datetime_loaded"]
        today = datetime.now(tz=timezone.utc).date()

        if max_date_loaded is not None and max_date_loaded > (today + timedelta(days=1)):
            logger.warning(
                f"[Watermark][{branch}] Ignoring corrupt date watermark "
                f"{max_date_loaded} because today is {today}"
            )
            max_date_loaded = None
            max_datetime_loaded = None

        # Defensive recovery:
        # A previously misparsed MDY file can poison max_datetime_loaded with an
        # impossible future date such as 2026-12-04 while max_date_loaded stays
        # at 2026-04-30. In that case, ignore the datetime watermark for reads
        # so the next run can recover using the date watermark.
        if (
            max_date_loaded is not None
            and max_datetime_loaded is not None
            and max_datetime_loaded.date() > (max_date_loaded + timedelta(days=1))
        ):
            logger.warning(
                f"[Watermark][{branch}] Ignoring corrupt datetime watermark "
                f"{max_datetime_loaded} because date watermark is {max_date_loaded}"
            )
            max_datetime_loaded = None

        return max_date_loaded, max_datetime_loaded
    except Exception as exc:
        logger.warning(f"Could not read watermark for {branch}: {exc}")
        return None, None
...
def update_branch_watermark(
    ...
    try:
        execute(
            ...
            VALUES (%s, %s, %s, NOW(), %s)
            ON CONFLICT (branch) DO UPDATE SET
                max_date_loaded     = CASE
                    WHEN branch_watermarks.max_date_loaded > CURRENT_DATE + 1
                    THEN EXCLUDED.max_date_loaded
                    ELSE GREATEST(
                ...
                max_datetime_loaded = CASE
                    WHEN EXCLUDED.max_datetime_loaded IS NULL
                    THEN branch_watermarks.max_datetime_loaded
                    WHEN branch_watermarks.max_datetime_loaded IS NULL
                    THEN EXCLUDED.max_datetime_loaded
                    WHEN branch_watermarks.max_date_loaded > CURRENT_DATE + 1
                    THEN EXCLUDED.max_datetime_loaded
                    WHEN branch_watermarks.max_datetime_loaded::date
                         > branch_watermarks.max_date_loaded + 1
                    THEN EXCLUDED.max_datetime_loaded
                    WHEN EXCLUDED.max_datetime_loaded > branch_watermarks.max_datetime_loaded
                    THEN EXCLUDED.max_datetime_loaded
...
# We choose the slash-date family (DMY vs MDY) per file because branches are
# currently mixed even within May 2026 daily exports.
_SALE_COMMON_DATETIME_FORMATS = [
    ...
    "%d-%b-%y %I:%M:%S %p",   # Text month 12hr:              06-May-26 7:23:24 PM
    "%d-%b-%Y %I:%M:%S %p",
    "%d-%b-%y %I:%M %p",
    "%d-%b-%Y %I:%M %p",
    "%d-%B-%y %I:%M:%S %p",
...
_SALE_DMY_DATETIME_FORMATS = [
    "%d/%m/%Y %H:%M:%S",      # DMY with seconds:             07/05/2026 18:51:27
    "%d/%m/%Y %H:%M",         # DMY no seconds:               05/05/2026 19:37
    "%d/%m/%Y",               # DMY date-only:                07/05/2026
    "%d/%m/%y %H:%M:%S",      # DMY 2-digit year + seconds:   01/05/26 19:35:00
    "%d/%m/%y %H:%M",         # DMY 2-digit year:             01/05/26 19:35
    "%d/%m/%y",               # DMY 2-digit date-only:        01/05/26
    "%d/%m/%Y %I:%M:%S %p",   # DMY 12hr:                     07/05/2026 7:04:18 PM
    "%d/%m/%Y %I:%M %p",
    "%d/%m/%y %I:%M:%S %p",
    "%d/%m/%y %I:%M %p",
]

_SALE_MDY_DATETIME_FORMATS = [
    "%m/%d/%Y %H:%M:%S",      # MDY with seconds:             5/7/2026 19:04:18
    "%m/%d/%Y %H:%M",         # MDY no seconds:               5/7/2026 19:04
    "%m/%d/%Y",               # MDY date-only:                5/7/2026
    "%m/%d/%y %H:%M:%S",      # MDY 2-digit year + seconds:   1/5/26 19:35:00
    "%m/%d/%y %H:%M",         # MDY 2-digit year:             1/5/26 19:35
    "%m/%d/%y",               # MDY 2-digit date-only:        1/5/26
    "%m/%d/%Y %I:%M:%S %p",   # MDY 12hr:                     5/7/2026 7:04:18 PM
    "%m/%d/%Y %I:%M %p",
    "%m/%d/%y %I:%M:%S %p",
    "%m/%d/%y %I:%M %p",
]

_DEFAULT_SALE_DATETIME_FORMATS = (
    _SALE_COMMON_DATETIME_FORMATS
    + _SALE_DMY_DATETIME_FORMATS
    + _SALE_MDY_DATETIME_FORMATS
)

_SALES_FORMAT_DECISION_CACHE: dict[tuple[str, int, int], tuple[list[str], str]] = {}


def _normalize_pos_datetime_text(value) -> str:
    return re.sub(r"\s+", " ", str(value).strip().strip("#").strip())


def _parse_sales_datetime_text(
    cleaned: str,
...
def _infer_sales_file_date_hint(path: Path) -> date | None:
    stem = path.stem
    for pattern in (
        r"sales_report_(\d{8})",
        r"(\d{8})",
...
def _score_sales_family(
    parsed_count: int,
    max_dt: datetime | None,
    file_date_hint: date | None,
) -> tuple[int, int, int]:
    if parsed_count <= 0:
        return (0, -10_000, 0)
    if max_dt is None or file_date_hint is None:
        return (1, 0, parsed_count)

    day_diff = (file_date_hint - max_dt.date()).days
    not_future = 1 if day_diff >= 0 else 0
    proximity = -abs(day_diff)
    return (not_future, proximity, parsed_count)


def _detect_sales_datetime_formats(path: Path, values: pd.Series) -> tuple[list[str], str]:
    stat = path.stat()
    cache_key = (str(path), stat.st_size, int(stat.st_mtime))
    cached = _SALES_FORMAT_DECISION_CACHE.get(cache_key)
    ...
    sample = (
        values.dropna()
        .astype(str)
        .map(_normalize_pos_datetime_text)
    )
    sample = sample[sample != ""].drop_duplicates()

    if sample.empty:
        result = (list(_DEFAULT_SALE_DATETIME_FORMATS), "DMY")
        _SALES_FORMAT_DECISION_CACHE[cache_key] = result
        return result

    file_date_hint = _infer_sales_file_date_hint(path)

    def _family_stats(formats: list[str]) -> tuple[int, datetime | None]:
        parsed_count = 0
        max_dt = None
        format_order = _SALE_COMMON_DATETIME_FORMATS + formats
        for raw in sample:
            parsed = _parse_sales_datetime_text(raw, format_order)
            if parsed is None:
                continue
            parsed_count += 1
            if max_dt is None or parsed > max_dt:
                max_dt = parsed
        return parsed_count, max_dt

    dmy_count, dmy_max = _family_stats(_SALE_DMY_DATETIME_FORMATS)
    mdy_count, mdy_max = _family_stats(_SALE_MDY_DATETIME_FORMATS)

    dmy_score = _score_sales_family(dmy_count, dmy_max, file_date_hint)
    mdy_score = _score_sales_family(mdy_count, mdy_max, file_date_hint)
    ...
    logger.info(
        f"[Sales] Date parser for {path.name}: {chosen_label} preferred "
        f"(file_date_hint={file_date_hint}, "
        f"DMY parsed={dmy_count} max={dmy_max.date() if dmy_max else None}, "
        f"MDY parsed={mdy_count} max={mdy_max.date() if mdy_max else None})"
    )

    result = (chosen_formats, chosen_label)
    _SALES_FORMAT_DECISION_CACHE[cache_key] = result
    return result


def _clean_date_sold(value, formats=None) -> date | None:
    """
    Parse Date Sold from POS format.
    Handles two formats:
      - Old ISO (with optional # guards): '#2026-03-21 08:33:11#' -> date(2026, 3, 21)
      - New DMY (DD/MM/YYYY h:mm[:ss] AM/PM): '11/04/2026 9:31:07 AM' -> date(2026, 4, 11)
    Uses explicit strptime list -- avoids pd.to_datetime dayfirst ambiguity.
    """
    if pd.isna(value):
        return None
    cleaned = _normalize_pos_datetime_text(value)
    if not cleaned:
        return None
    parsed = _parse_sales_datetime_text(cleaned, formats or _DEFAULT_SALE_DATETIME_FORMATS)
    return parsed.date() if parsed else None


def _clean_datetime_sold(value, formats=None) -> datetime | None:
    """
    ...
    Handles two formats:
      ...
      - New DMY (DD/MM/YYYY h:mm[:ss] AM/PM): '11/04/2026 9:31:07 AM'
        -> datetime(2026, 4, 11, 9, 31, 7)
    DMY date-only / no-seconds values are still accepted and default missing
    seconds to 0, which is fine
    ...
    if not cleaned:
        return None
    return _parse_sales_datetime_text(cleaned, formats or _DEFAULT_SALE_DATETIME_FORMATS)
...
def parse_sales_file(path: Path) -> pd.DataFrame:
    ...
    - Validates all REQUIRED columns are present (original 10 columns)
    - New columns (Unit Cost, Tax Amount, etc.) are filled with None if absent
      so old files continue to load without errors (backward compat)
    ...
    missing = [c for c in SALES_REQUIRED_COLUMNS if c not in df.columns]
    if missing:
        raise ValueError(f"Sales file {path.name} missing columns: {missing}")

    df = df[SALES_COLUMNS].copy()

    sale_formats, _ = _detect_sales_datetime_formats(path, df["Date Sold"])

    df["Date Sold Raw"]      = df["Date Sold"].copy()   # keep original
    df["Date Sold Datetime"] = df["Date Sold Raw"].apply(
        lambda value: _clean_datetime_sold(value, sale_formats)
    )
    ...
    df["Total Sales Amount"] = pd.to_numeric(df["Total Sales Amount"], errors="coerce")
    df["Total Cost"]         = pd.to_numeric(df["Total Cost"],         errors="coerce")
    df["Transaction ID"]     = df["Transaction ID"].astype(str).str.strip()
    df["Item"]               = df["Item"].astype(str).str.strip()
    # Sales Rep ID kept as string (POS IDs can have leading zeros)
...
def parse_sales_file_after_date(
    ...
    def _keep_row(row) -> bool:
        ...
        d = row.get("Date Sold")
        return not pd.isna(d) and d > after_date
...
def _insert_sales_rows(conn, df: pd.DataFrame, branch: str) -> int:
    ...
    for _, row in df.iterrows():
        rows.append((
            ...
            _to_numeric(row.get("Unit Cost")),
            _to_date(row.get("Last Sold")),
            _to_numeric(row.get("Qty Sold")),
            _to_numeric(row.get("Total (Tax Ex)")),
            _to_numeric(row.get("Tax Amount")),
            _to_numeric(row.get("Total Sales Amount")),
            _to_numeric(row.get("Total Cost")),
            ...
            _to_str(row.get("Sales Rep ID")),
            _to_str(row.get("Sales Rep Name")),
        ))
