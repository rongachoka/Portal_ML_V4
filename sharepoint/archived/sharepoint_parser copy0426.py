...
def get_max_date_sold(path: Path) -> date | None:
    ...
    Reads only the Date Sold column — avoids loading the full file into memory.
    ...
    try:
        ...
        if suffix == ".csv":
            try:
                df = pd.read_csv(
                    path, usecols=["Date Sold"], dtype=str, low_memory=False
                )
            except UnicodeDecodeError:
                df = pd.read_csv(
                    path, usecols=["Date Sold"], dtype=str,
...
def _clean_date_sold(value) -> date | None:
    ...
    try:
        # ISO format (YYYY-MM-DD...) — dayfirst must be False, it's unambiguous
        if re.match(r'^\d{4}[-/]\d{2}', cleaned):
            return pd.to_datetime(cleaned, dayfirst=False).date()
        # DD/MM/YYYY format (ABC branch) — dayfirst=True
    except Exception:
        return None
...
def _clean_datetime_sold(value) -> datetime | None:
    ...
    Strips leading/trailing # characters.
    e.g. '#2026-03-21 08:33:11#' → datetime(2026, 3, 21, 8, 33, 11)
