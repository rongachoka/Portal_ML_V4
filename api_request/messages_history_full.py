...
def extract_message_timestamp(status_array: list) -> str:
    ...
    and formats it. Safely handles string/float casting.
    """
    if not status_array:
        return ""
    ...
    if target_ts:
        try:
            # Safely cast to float to prevent TypeError
    ...
    while url:
        try:
            ...
            for item in items:
                ...
                # If we've seen this message before, stop fetching
...
def main():
    ...
    print(f"Loaded {len(existing_ids)} existing messages to skip.")
