...
OUTPUT_FILE  = BASE_DIR / "data" / "03_processed" / "sales_attribution" / "social_sales_direct_TEST.csv"
CONCERN_OUTPUT_FILE = OUTPUT_FILE.parent / "social_sales_by_concern_TEST.csv"
...
def run_social_sales_direct():
    ...
    rev_col    = "Total (Tax Ex)"
    month_rev  = df_month[rev_col].sum()
    month_txns = df_month["Transaction ID"].nunique()
    month_label = now.strftime("%B %Y")
    total_rev  = df_final[rev_col].sum()
